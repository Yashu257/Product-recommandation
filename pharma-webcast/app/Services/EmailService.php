...
final class EmailService
{
     ...
     * @param array $event         Event DB row
     */
    public function sendRegistrationConfirmation(array $registration, array $event): int
    {
        return $this->dispatch(
            fn() => (new RegistrationConfirmationMail($registration, $event))->queue()
        );
    }
    ...
    /**
     * Sent synchronously (user is waiting on the page).
     *
     * @param array  $user  ['first_name', 'last_name', 'email']
     * @param string $token Cryptographically secure reset token
     */
    public function sendPasswordReset(array $user, string $token, int $expiresInMinutes = 60): bool
    {
        try {
            (new PasswordResetMail($user, $token, $expiresInMinutes))->send();
            return true;
        } catch (Throwable $e) {
            $this->logger->error('PasswordResetMail failed', [
                'email' => $user['email'] ?? '',
                'error' => $e->getMessage(),
            ]);
            return false;
