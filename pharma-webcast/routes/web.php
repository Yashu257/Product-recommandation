...
$router->post('/login',                     'Auth\LoginController@login',                ['guest', 'csrf', 'throttle:5,1']);
$router->post('/logout',                    'Auth\LoginController@logout',               ['auth',  'csrf']);

// --- Registration (Account) --------------------------------------------------
$router->get( '/register',                  'Auth\RegisterController@showForm',          ['guest']);
$router->post('/register',                  'Auth\RegisterController@register',          ['guest', 'csrf', 'throttle:3,1']);

// --- Email Verification ------------------------------------------------------
$router->get( '/verify-email/{token}',      'Auth\VerificationController@verify');
$router->post('/verify-email/resend',       'Auth\VerificationController@resend',        ['auth', 'csrf', 'throttle:3,10']);

// --- Password Reset ----------------------------------------------------------
$router->get( '/forgot-password',           'Auth\PasswordResetController@showRequest',  ['guest']);
$router->post('/forgot-password',           'Auth\PasswordResetController@sendLink',     ['guest', 'csrf', 'throttle:3,10']);
$router->get( '/reset-password/{token}',    'Auth\PasswordResetController@showReset',    ['guest']);
$router->post('/reset-password',            'Auth\PasswordResetController@reset',        ['guest', 'csrf']);
